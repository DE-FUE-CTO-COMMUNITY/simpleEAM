...
Object.entries(originalData).forEach(([tabName, tabData]) => {
  ...
  updatedTabData.forEach(row => {
    if (row.owners || row.parents || row.supportsCapabilities) {
      console.log(`  Entity ${row.id} (${row.name || row.firstName + ' ' + row.lastName}) would update relationships:`);
      ...
      if (row.supportsCapabilities) {
        ...
        console.log(`    supportsCapabilities: ${capabilityIds.join(', ')} -> ${actualCapabilityIds.join(', ')}`);
      }
    }
  });
});
...
console.log('✅ After fix: Relationship updates use actual database IDs (db-uuid-bc-001, db-uuid-app-001, db-uuid-p-001)');
