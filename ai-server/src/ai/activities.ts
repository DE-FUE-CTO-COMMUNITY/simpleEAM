...
// ─────────────────────────────────────────────
// Sovereignty Score Calculation Activities
// ─────────────────────────────────────────────
  ...
  await graphqlRequest({
    query: `
      mutation UpdateSovereigntyScores($id: ID!, $update: CompanyUpdateInput!) {
        updateCompanies(where: { id: { eq: $id } }, update: $update) {
          companies { id }
        }
      }
    `,
...
export async function markSovereigntyCalculating(input: {
  ...
  await graphqlRequest({
    ...
    variables: {
      id: input.companyId,
      update: { sovereigntyScoreStatus: { set: 'CALCULATING' } },
    },
    accessToken: input.accessToken,
  })
}
...
export async function markSovereigntyError(input: {
  ...
  await graphqlRequest({
    ...
    variables: {
      id: input.companyId,
      update: { sovereigntyScoreStatus: { set: 'ERROR' } },
    },
    accessToken: input.accessToken,
  })
}
