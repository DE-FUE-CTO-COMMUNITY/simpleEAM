...
const parseAudience = (audience: unknown): string[] => {
...
const isAudienceAccepted = (decoded: jwt.JwtPayload): boolean => {
  ...
  const tokenAudiences = parseAudience(decoded.aud)
  if (tokenAudiences.some(audience => allowedAudiences.includes(audience))) {
    return true
  }

  const authorizedParty = typeof decoded.azp === 'string' ? decoded.azp : ''
  if (authorizedParty && allowedAudiences.includes(authorizedParty)) {
    return true
  }
  ...
  return new Promise((resolve, _reject) => {
    jwt.verify(
      ...
      (err, decoded) => {
        ...
        if (!isAudienceAccepted(payload)) {
          console.error('Token verification error: audience/azp not allowed', {
            aud: payload.aud,
            azp: payload.azp,
            expected: getAllowedAudiences(),
          })
          return resolve(null)
        }
