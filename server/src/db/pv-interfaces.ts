...
export async function createInterfaceOwnership(session: Session) {
  ...
  await session.run(`
    ...
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(sap_to_salesforce)
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(salesforce_to_hubspot)
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(sap_to_mes)
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(mes_to_quality)
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(quality_to_sap)
  `)
  ...
  await session.run(`
    ...
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(plm_to_mes)
    CREATE (enterprise_architect)-[:RESPONSIBLE_FOR]->(rd_to_plm)
